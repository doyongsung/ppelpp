	...
	public String getCrewList(
		...
		model.addAttribute("myCrewList", myCrewList);
		  ...
		  //크루 검색기능 더해서 전체 크루 리스트 처리 
			/*
			 * List<Crew> list = null; if(searchType.getKeyword() !=null &&
			 * searchType.getKeyword().trim().length() > 0) { list=
			 * service.getCrewListAll(searchType); } else { list = service.getCrewListAll();
