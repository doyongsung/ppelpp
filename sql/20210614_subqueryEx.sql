...
from emp e, emp m
where e.job = m.job and e.empno = 7788 
;
...
select e.deptno, empno,job ,ename
from emp e ,dept d
...
from emp
where deptno in (select deptno from emp where ename like '%M%') and sal > (select avg(sal) from emp)
;
...
select empno , ename , sal
from emp
where sal > (select avg(sal)from emp) and deptno in 
